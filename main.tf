...
data "aws_subnets" "t_my_subnets" {
  filter {
    name = "vpc-id"
    values = [ aws_vpc.t_cr_vpc.id]
  }
...
data "aws_subnet" "t_my_subnet_ids" {
  for_each = { for index, subnetid in data.aws_subnets.t_my_subnets.ids : index => subnetid }
  id       = each.value
}
...
module "eks_cluster" {
  ...
  subnets = data.aws_subnets.t_my_subnets.ids
